...
class CodeIdentityManager extends BaseManager
{
	...
	public function getWordPressCodeInfo( $auditsFilesDirectory ) {
		...
		if ( isset( $plugin_theme_header['ThemeURI'] ) ) {
			$codeInfo['type'] = 'theme';
		} else if ( isset( $plugin_theme_header['PluginURI'] ) ) {
	...
	public function get_plugin_theme_header( $auditsFilesDirectory, $directory_scanned = false ) {
		...
		if ( ! empty( $files ) ) {
			foreach ( $files as $file ) {
				...
				} else if ( $directory_scanned && 'style.css' === $file && is_readable( $file_path ) ) {
					$header = $this->get_theme_header( $file_path );
				} else if ( 'php' === $file_extension && is_readable( $file_path ) ) {
